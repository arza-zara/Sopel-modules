...
def yle_tulostus(bot, trigger):
    ...
    # Yrittaa hakea artikkelin sisallon, sen otsikon ja linkin. Jos ei pysty,
    # postaa etta paska input.
    try:
        artikkeli = resp['responseData']['feed']['entries'][
            int(trigger.group(2).split(" ")[0]) - 1]['content'].split("</p>")
        title = resp['responseData']['feed']['entries'][
            int(trigger.group(2).split(" ")[0]) - 1]['title']
        # Lyhentaa urlin poistamalla artikkelin nimen urlista
        link = resp['responseData']['feed']['entries'][int(trigger.group(2).split(" ")[0]) - 1]['link'].split("uutiset")[0] + "uutiset/" + \
            resp['responseData']['feed']['entries'][
                int(trigger.group(2).split(" ")[0]) - 1]['link'].split("/")[-1].split("?")[0]
        if len(artikkeli) < 7:
            ...
            bot.say("00" + title + "" + "(" + link + ")")
            for paragraph in artikkeli:
                bot.msg(trigger.nick, paragraph.replace("<p>", "")
                        .replace("<h3>", "00").replace("</h3>", ": "))
            return
    except (ValueError, UnicodeEncodeError, IndexError):
        bot.reply(
            "Paska input. Komento on: <.ylep|.ylet> [1..4|1..12] [lue|cont|content]")
        return
...
def yle_uusimmat(bot, trigger):
    ...
    # kay lapi artikkelit, tallentaa titlen ja linkin artikkeliin minka
    # jalkeen lisaa ne outputtiin
    for otsikko in range(first_range, second_range):
        title = resp['responseData']['feed']['entries'][otsikko]['title']
        link = resp['responseData']['feed']['entries'][otsikko]['link'].split(
            "uutiset")[0] + "uutiset/" + resp['responseData']['feed']['entries'][otsikko]['link'].split("/")[-1].split("?")[0]
        output.append("00" + str(otsikko + 1)
                      + ". " + title + " (" + link + ")")

    # artikkelin numero (1-12)
    position = str(first_range + 1) + "-" + \
        str(second_range) + "/" + str(total)
