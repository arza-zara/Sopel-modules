...
def quote_add(bot, trigger):
    ...
    #generates the new quote string (I%20am%20a%20quote|Me%20too|I%20am%20as%20well|)
    quote = orig + args + '|'
...
def get_conv(bot, trigger):
    ...
    #gets all the quotes, removes the trailing "|", and then splits the quotes into quote_list
    ...
    #if a quote number is given, this fetches the quote associated with the given number.
    if trigger.group(2):
    ...
    #converts the "%20" etc. back into a readable format (7: This is the quote)
...
def quote_del(bot, trigger):
    ...
    #This if-bit allows only me (meicceli) to delete all the quotes, but allows others to delete one quote per 60 seconds (rate is 60)
    ...
    #deletes the item by index shown in the beginning of every quote (for example "5: quote" could be deleted with .delquote 5)
    del quote_list[int(del_key) - 1]

    #This bit makes sure that there won't be any "||" found, which would mess up the splitting in .quote (there would be empty strings in the quote_list)
    modified_list = ""
    for i in quote_list:
        modified_list += i + "|"
...
def quote_search(bot, trigger):
    ...
    #if ", " found, stores the search number into search_number and the query into search_term. If no ", " found, splits search terms with a space (%20)
    if args.find(', ') != -1:
    ...
    try:
        output = "(" + str(findings.index(findings[search_number - 1]) + 1) + "/" + str(len(findings)) + ") - " + str(quote_list.index(findings[search_number - 1]) + 1) + ": " + findings[search_number - 1]
    except IndexError:
